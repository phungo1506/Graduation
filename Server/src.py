...
from deploy.pptracking.python.mot_jde_infer import JDE_Detector
import base64
import time
import operator
...
def web_thuan():
    ...
    if file.filename == '':
        ...
        detector = JDE_Detector(model_dir = phu_checkpoint, device='GPU', output_dir=thuan_output_path, save_mot_txts=thuan_output_path)     
        out_file = detector.predict_video(video_file = thuan_input_path + str(filename), camera_id = -1)
        #print(thuan_input_path + str(filename))
        #print(out_file)
